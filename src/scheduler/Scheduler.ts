...
import { getActivePattern, STEP_TICKS } from "../project-model/types";
...
export class Scheduler {
  ...
  start(): void {
    ...
    let position = transport.position % patternTicks;
    if (position < 0) position += patternTicks;
    this.windowStartTick = position;
    this.stopped = false;
  ...
  private tick(): void {
    ...
    if (windowEnd <= this.windowStartTick) {
    ...
    const firstStep = Math.ceil(windowStart / STEP_TICKS - 1e-9);
    for (let t = firstStep * STEP_TICKS; t < windowEnd; t += STEP_TICKS) {
      const when = transport.timeAtTick(t);
      if (when < now - 0.002) continue;
      const stepIndex = Math.floor(t / STEP_TICKS) % pattern.stepCount;
      this.scheduleDrums(doc, stepIndex, when);
    }

    const rel = ((windowStart % patternTicks) + patternTicks) % patternTicks;
    for (const track of doc.tracks) {
      if (track.kind !== "instrument") continue;
      const notes = pattern.notes?.[track.id];
      if (!notes || notes.length === 0) continue;
      for (const note of notes) {
        const occurrence = windowStart + ((note.start - rel + patternTicks) % patternTicks);
        if (occurrence >= windowEnd) continue;
        const when = transport.timeAtTick(occurrence);
        if (when < now - 0.002) continue;
  ...
  private scheduleDrums(doc: ProjectDocument, stepIndex: number, when: number): void {
    const pattern = getActivePattern(doc);
