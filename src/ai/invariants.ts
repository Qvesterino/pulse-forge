...
export function inspectPatternInvariants(
  ...
  for (const [padId, row] of Object.entries(pattern.rows ?? {})) {
    if (!padIds.has(padId)) issue(issues, "unknown-pad", `rows.${padId}`, "row references a pad that is not in the project");
    if (!Array.isArray(row) || row.length !== stepCount) {
      issue(issues, "row-length", `rows.${padId}`, `expected ${stepCount} steps, got ${Array.isArray(row) ? row.length : "non-array"}`);
      continue;
    }
  ...
  for (const [trackId, notes] of Object.entries(pattern.notes ?? {})) {
    if (!instrumentIds.has(trackId)) issue(issues, "note-track", `notes.${trackId}`, "notes reference a non-instrument track");
  ...
  for (const [padId, steps] of Object.entries(pattern.stepMeta ?? {})) {
    ...
    for (const [stepKey, meta] of Object.entries(steps)) {
      ...
      if (meta.probability !== undefined && (!Number.isFinite(meta.probability) || meta.probability < 0 || meta.probability > 1)) {
      ...
      if (meta.microtiming !== undefined && (!Number.isFinite(meta.microtiming) || meta.microtiming < -1 || meta.microtiming > 1)) {
  ...
  for (const [index, bar] of (pattern.phrasePlan ?? []).entries()) {
    ...
    if (!Number.isInteger(bar.bar) || bar.bar < 0 || !Number.isInteger(bar.startStep) || !Number.isInteger(bar.endStep)
      || bar.startStep < 0 || bar.endStep <= bar.startStep || bar.endStep > stepCount
      || !PHRASE_SECTIONS.has(bar.section) || bar.startStep < previousEnd) {
      issue(issues, "phrase-plan", path, "phrase bars must be ordered, non-overlapping, bounded and use a known section");
    }
  ...
  if (pattern.generation?.outputContentHash && pattern.generation.outputContentHash !== expectedHash) {
    issue(issues, "content-hash", "generation.outputContentHash", "generation output hash does not match musical content");
  }
...
export function assertPatternInvariants(
  ...
  if (!report.ok) {
    throw new Error(`Pattern invariant failure: ${report.issues.map((item) => `${item.code}@${item.path}`).join(", ")}`);
  }
}
