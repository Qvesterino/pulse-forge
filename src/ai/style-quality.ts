...
function referenceMetrics(groove: GrooveData, stepCount: number): StyleDistanceMetrics {
  const rows = groove.activePads.map(padIndex => {
    ...
    for (let step = 0; step < stepCount; step++) {
      row[step] = referenceAt(groove.patterns.map(pattern => pattern[padIndex] ?? []), step);
    }
    return row;
  });
...
export function evaluateStyleDistance(
  ...
  const distance = round(
    densityDistance * 0.4
    + syncDistance * 0.3
    + downbeatDistance * 0.2
    + velocityDistance * 0.1,
  );
...
function budgetForRole(role: PadRole): SyncopationBudget {
  switch (role) {
    case 'kick': return { min: 0, max: 0.28 };
    case 'snare':
    case 'clap': return { min: 0.03, max: 0.38 };
    case 'closedHat': return { min: 0.2, max: 0.82 };
    case 'openHat':
    case 'perc': return { min: 0.12, max: 0.72 };
    case 'tom': return { min: 0.04, max: 0.55 };
    default: return { min: 0.05, max: 0.7 };
  }
}
...
export function enforceSyncopationBudget(
  row: number[],
  ...
  if (score < budget.min) {
    const candidates = row
      .map((velocity, step) => ({ velocity, step, reference: referenceAt(references, step), weight: syncopationWeight(step) }))
      .filter(candidate => candidate.velocity <= 0 && candidate.reference > 0 && candidate.weight > 0.5 && !isAnchor(role, candidate.step))
