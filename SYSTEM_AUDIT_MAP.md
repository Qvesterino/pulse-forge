...
## 17. Boundary & ownership findings (GOAL 02, 2026-09-20 — refreshed 2026-09-25, re-run 4)

1. **Command-layer exclusivity: HOLDS** (see §3). Re-verified 2026-09-25 over the NEW intent surfaces: conversation faders/tempo (IntentPanel → `store.execute(applyFaderIntent/applyTempoIntent(...))`), producer-session (pure module, no doc access), briefFixes (transient React state merged into the generation input; generation itself is command-wrapped). Enforcement via dev-only deepFreeze of undo snapshots.
2. **DI bypasses (UI constructs infra directly): RESOLVED 2026-09-25.** `MorphPresetRepository` + `UltinaPresetRepository` now live on `CoreServices` (`services.morphPresets`/`ultinaPresets`, shared instances — one preset cache per app instead of one per panel callback) and all 8 panel construction sites route through `useServices()`. Same pattern as the GroovePoolRepository fix. Test mocks: `tests/helpers.tsx` mockServices + `tests/ui/ProjectBrowser.test.tsx` makeCore carry stubs.
3. **Storage keys:** `pf-publish-code` single-source since re-run 3. Naming conventions coexist (`pf-kebab` vs `pf:colon`) — cosmetic.
4. **Output-trim clamp duplication: RESOLVED re-run 3** — `schema.ts:703` calls `clampFxOutputTrimDb`.
5. **Provenance walk duplication:** fixed by `a91ad77`; contract pinned by `tests/gallery-intent-contract.test.ts`.
6. **intent/ ↔ commands/ cycle: CLOSED then PINNED (2026-09-25).** The monitored accident happened: `pipeline → ranking-v3 → audition → commands` (audition needs `foldFxIntoDoc` for ghost docs). Runtime-safe by construction — ranking-v3 loads audition via **dynamic** import, so module evaluation never re-enters commands — but breaking it would drag the whole command layer (foldProductionActions needs addEffect/setEffectParam/setBeatManglerSteps). Pinned by `tests/architecture-cycles.test.ts`: if the dynamic import goes static, the test fails. Full cycle allowlist (5 SCCs, all documented: packCode both-edges-type-only, schema↔templates↔template-pack2, metering↔kweighting, UserSample↔RecordingRecovery type-only back edge, sections↔song type-only back edge) lives in that test.
7. **Exhaustiveness guards EXIST for both orders:** `tests/effects.test.ts:13` (EFFECT_ORDER) + `tests/instrument-definitions.test.ts:20` (INSTRUMENT_ORDER).
8. **Provider candidates leaf (new, 2026-09-25):** `attachProvenance`/`candidatePlan`/`evaluateCandidate`/`invariantErrors` extracted from `intent/providers/local.ts` into `intent/providers/candidate.ts`, breaking the local↔symbolic value cycle; symbolic must never import local (pinned in the cycle test). local→symbolic remains sanctioned (template-provider fallback branch).
