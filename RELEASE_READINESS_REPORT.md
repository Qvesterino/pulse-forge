...
The latest reviewed release candidate is `70cd4e9`, with the functional
...
browser suite now has an explicit negative-route guard. `70cd4e9` additionally
forwards scheduled continuous MOD A/B amount writes into the amount gains of
already-playing fallback voices; selector topology remains note-bound by
contract.
...
The latest quiet Chromium smoke from the current candidate `70cd4e9` is **221/221**:
...
production-config preflight also pass. PRISM A/B persistence, live preview/history, source
...
| `npm run build` + bundle budgets               | **PASS**                                                                     | current candidate `70cd4e9`: entry 940 KB / 995 budget; total JS 1851 KB / 2400 budget; core worklets 98 KB / 120 budget; 356 modules; PWA precache 57 entries / 4040.86 KiB; lazy ranker WASM is 13.6 MB and is not precached                                                                             |
...
| `npm run test:browser` (latest Chromium smoke) | **PASS — 221/221 Chromium (current `70cd4e9`)**                              | All audio/DSP/FXEQ checks passed, including the negative AMP/CUTOFF guard and held-voice MOD amount automation; the existing latency, PRISM, collab, embed/share and touch checks also passed. No threshold was changed. |
...
browser candidate also has a clean `221/221` rerun on `70cd4e9`; no threshold was changed.
...
| R15 | Closed in `2242541` | PRISM host workflow integration is implemented: persisted A/B slots hydrate runtime morph state; PRISM sliders use preview + cancel rollback; source picker rewires sidechain; tempo-sync selects and LR transfer overlay are now user-facing; async history requests are queued                             | targeted host/worklet/processor `45/45`, UI/curve `13/13`, typecheck/build, current PRISM browser workflow `221/221`, full Vitest `236/2320/103`, production smoke                                                                                                         | no remaining known implementation gap in the reviewed PRISM workflow; release approval still depends on manual/device, deployed-host and formatting owner gates        | retain targeted regressions and execute the remaining owner gates from `20f63ae`                                                                         | **No — see R6, DEP-01 and FMT-01**                                                                         |
...
1. Review/tag `70cd4e9` as the current KYX candidate; its functional/test baseline is `8f11255`, containing the FXEQ crossover correction on top of the reviewed Ultina reconciliation in `173f5ce`, plus the synchronized generated PRISM worklet artifact, scorepack duration/error-boundary fixes in `dad84f3`/`a9dd506`, deterministic sparse instrument preset application in `954739e`, bounded fallback modulation routes from `326d443`, and live held-voice MOD amount automation. Matching upstream Ultina source/test is committed as `c0a549d`. Keep `e701b05` as the immutable historical comparison baseline.
2. The authoritative full Vitest baseline is green at `236` files / `2320` passed / `103` skipped / `2423` total; post-candidate scorepack is `4/4`, current quiet Chromium is `221/221`, build and production smoke pass, and the ranker fallback unit/browser coverage plus stalled-manifest regression pass. Preserve the thresholds and rerun the full suite as part of the final owner release procedure if the release process requires a post-candidate full-suite artifact.
