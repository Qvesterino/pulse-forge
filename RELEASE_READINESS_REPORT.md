...
The latest reviewed implementation candidate is `b8c7a00`, with the functional
...
`c0a549d`. The implementation commits through `b8c7a00` are clean and
reviewable in `8f11255` + `dfaf230` plus the later hardening commits; `b8c7a00`
also reconciles the metronome/count-in transport WIP with dedicated behavior
coverage for lead-in/content timing, seek/loop reset, project-bar timing,
remote sync and TAP tempo.
...
navigation and slicing. `b8c7a00` reconciles the metronome/count-in WIP into
the clean candidate, including lead-in/content gating, project-bar timing,
remote-sync bypass, early-position preservation and TAP-tempo coverage.
...
`late=0.1507`.
The current full-tree Vitest run on `b8c7a00` passed **239 files, 2351 tests
and 103 skipped** (`2454` total) in `424.94s`, including the browser
compatibility contract, FXEQ performance gate and all 300-second FXEQ/VLYX/VØID
soaks. The historical pre-`5b2bf5b` single-worker baseline remains
`236/2320/103`; the post-fix targeted PRISM/Ultina/FXEQ batch is **115/115**.
...
targeted/UI/browser-tested. Release remains blocked only by the owner gates
below:
...
tail peak. The 300-second FXEQ/VLYX/VØID soak tests also passed in the current
full attempt.
...
| Full Vitest suite                              | **PASS — 239 files / 2351 tests passed / 103 skipped / 2454 total** | `424.94s`; includes browser-compat, FXEQ performance, all 300 s FXEQ/VLYX/VØID soaks and no threshold changes |
| `npm run build` + bundle budgets               | **PASS — candidate build after `b8c7a00`**                                | entry 946 KB / 995 budget; total JS 1863 KB / 2400 budget; core worklets 98 KB / 120 budget; 359 modules; PWA precache 57 entries / 4052.78 KiB; lazy ranker WASM is 13.6 MB and is not precached |
...
| current lifecycle/async regression batch      | **PASS — 40/40**                                                               | Closed-context recreation/worklet refresh lock, recorder partial-start/stop/cancel cleanup, onset Worker fallback/copy/filter/abort, factory audio quality and browser feature detection. |
...
_Full-run status:_ the current full attempt completed in `424.94s` with
**239 files passed / 2351 tests passed / 103 skipped / 2454 total**. FXEQ
performance passed in the full run at `15.5×` median and `11.3×` p95 versus
the `22×`/`25×` budgets; both 300-second plugin soaks passed. The focused
post-crossover PRISM batch is `115/115`, and the current lifecycle/compatibility
batch is `40/40`. Older timing failures remain historical evidence only. No
...
flow passed **226/226 in Chromium, Firefox and Microsoft Edge** on `b8c7a00`,
...
| R7  | Closed in `8f11255` + `dfaf230` | Local Ultina vendor/worklet result remains committed in `173f5ce`, matching upstream source/test is committed in `D:/VocalForge_DAW` as `c0a549d`, and the FXEQ six-band crossover correction plus generated PRISM artifact are committed in `8f11255` + `dfaf230`; lifecycle/async hardening is in `5b2bf5b`, and transport/UI reconciliation is in `b8c7a00` | `git show --stat 8f11255`; `git show --stat dfaf230`; source/vendor equality check; upstream affected suite `105/105`; focused PRISM batch `115/115`; current browser `226/226` in Chromium/Firefox/Edge; current full Vitest `239/2351/103`; clean candidate tree | the candidate scope is immutable; remaining work is manual/deployed/formatting owner decisions | retain the split commit history and run owner release gates from `20f63ae` | **No — WIP-01 is reconciled; R6, DEP-01 and FMT-01 remain** |
...
| R12 | Closed | FXEQ render-path allocations in oversampler/limiter and reverb morph coefficient rebuilds were removed; thresholds were unchanged                                                                                                                                                    | `tests/fxeq-performance-gates.test.ts` is `3/3` in isolation and in the current full suite (`15.5×` median / `11.3×` p95 vs `22×`/`25×` budgets); focused PRISM batch `115/115`; 8/8 golden cases within tolerance (7/8 bit-exact, one documented legacy split difference); current browser CPU/PDC checks; build + production smoke | no known allocation regression in the validated PRISM path | retain the gate and rerun it from the release commit | No |
| R13 | Closed              | Instant Jam transport awareness now validates untrusted payloads before applying BPM/tick anchors; remote play starts the scheduler after re-anchoring and pause/stop always halts it; followers bypass local count-in when joining a live timeline                                                                                               | `tests/collab-transport.test.ts` 11/11; collab subset and full Vitest pass; post-change `npm run typecheck:clean` PASS                                                                                                                                                   | malformed pulses cannot poison playhead state and valid remote play/pause controls the actual scheduler lifecycle                                                    | retain malformed-payload, transition and no-local-lead-in coverage                                                                                                         | No                                                                                                             |
...
| R15 | Closed in `2242541` | PRISM host workflow integration is implemented: persisted A/B slots hydrate runtime morph state; PRISM sliders use preview + cancel rollback; source picker rewires sidechain; tempo-sync selects and LR transfer overlay are now user-facing; async history requests are queued                             | targeted host/worklet/processor `45/45`, UI/curve `13/13`, typecheck/build, current PRISM browser workflow `226/226` per Chromium/Firefox/Edge, factory audio `199/199`, production smoke                                                                                                         | no remaining known implementation gap in the reviewed PRISM workflow; release approval still depends on manual/device, deployed-host and formatting owner gates        | retain targeted regressions and execute the remaining owner gates from `20f63ae`                                                                         | **No — see R6, DEP-01 and FMT-01**                                                                         |
| R16 | **Closed in `b8c7a00`** | Metronome/count-in WIP is committed with dedicated behavior for lead-in clicks, content gating, seek/loop reset, time-signature bar length, early-position starts, remote sync and TAP tempo | `tests/scheduler.test.ts` 46/46; `tests/transport.test.ts` 29/29; `tests/collab-transport.test.ts` 11/11; `tests/ui/TopBar.test.tsx` 18/18; `npm run typecheck:clean`; current full Vitest 239/2351/103 | clean candidate tree; content cannot leak into count-in and remote followers are not delayed by local preferences | retain the regressions and verify the owner gates from §5 | **No — tree reconciliation complete** |
...
1. Review/tag `b8c7a00` as the current KYX implementation candidate; it includes the reviewed `e8c1ac9` factory-audition fixes plus closed-context recovery, context-keyed worklet refresh locking, deterministic recorder cleanup, cancel-safe onset analysis and the reconciled metronome/count-in transport UX. Its functional/test baseline is `8f11255`, with matching upstream Ultina source/test committed as `c0a549d`. Keep `e701b05` as the immutable historical comparison baseline. The candidate tree is clean; remaining release gates are manual/device, deployed-host and formatting decisions.
2. The current full Vitest run is green at `239` files / `2351` passed / `103` skipped (`2454` total); FXEQ performance passed at `15.1×` median and `14.5×` p95 in this run versus `22×`/`25×` budgets, and all 300-second plugin soaks passed. The lifecycle/compatibility targeted batch is `40/40`; factory preview audio QA is `199/199`, browser verification is `226/226` in Chromium/Firefox/Edge, scorepack is `4/4`, and production smoke/build plus explicit production preflight pass. Preserve thresholds and rerun from the final release commit if the release process creates a new candidate.
