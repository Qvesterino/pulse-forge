...
**Reviewed baseline:** current implementation candidate `5b2bf5b` (`fix: harden
audio lifecycle and async analysis`) on top of `e8c1ac9` (`fix: harden factory
preset auditions`), `601eb6c` (`fix: lazily activate fallback modulation
routes`) and `91077da` (`feat: improve preset discovery metadata`) and
`d4d974b` (`fix: live-update fallback modulation selectors`) on top of
`70cd4e9` (`fix: update fallback modulation amounts live`) and `326d443`
(`fix: bound fallback modulation routes`) and `954739e` (`fix: make sparse
instrument presets deterministic`) with functional/test baseline `8f11255` — FXEQ/PRISM six-band
...
hardening is in `f3514a5`, the browser fallback probe is in `f07a4e2`, and the
...
factory presets. `5b2bf5b` hardens context recreation after a closed
`AudioContext`, keys worklet refresh locks to the active context, makes recorder
start/stop/cancel cleanup deterministic, and moves long onset analysis behind a
cancel-safe Worker fallback for transient navigation and slicing. The
implementation scope is now reconciled in `b8c7a00`, which closes the
metronome/count-in WIP with dedicated lead-in, content-gating, seek/loop,
time-signature, remote-sync and TAP-tempo coverage. The current candidate tree
is clean. The current full browser verifier is green in Chromium, Firefox and
Edge at `226/226` per engine; the factory-only verifier is `199/199`. The
authoritative current-tree Vitest run is `239/2351/103` (`2454` total) and
passed, including the FXEQ/VLYX/VØID soak coverage. The post-candidate
scorepack regression is `4/4`; the current production browser smoke and
explicit production preflight also pass. Release approval still waits for the
manual device matrix, a real public deploy URL and the formatting decision.
...
| `WIP-01`   | **CLOSED — `b8c7a00`**                          | Release hygiene | Reconcile metronome/count-in, click lead-in and transport shortcut behavior before release                                                                                                  | `src/scheduler/Scheduler.ts`, `src/services.ts`, `src/transport/Transport.ts`, `src/collab/transportSync.ts`, `src/ui/TopBar.tsx`, `tests/scheduler.test.ts`, `tests/transport.test.ts`, `tests/collab-transport.test.ts`, `tests/ui/TopBar.test.tsx`                                                                                                                                                                  | typecheck; scheduler `46/46`, transport `29/29`, collab transport `11/11`, TopBar `18/18`; full Vitest `239/2351/103`; collab followers bypass local lead-in, early starts preserve position, and project bar length drives future lead-in |
...
6. **Browser acceptance — PRISM flow DONE; global release gates open.** Browser check teraz
   ...
   samostatný browser suite zároveň prešiel real-worklet DSP, PDC, metery,
    sidechain render, export determinism a zero-error flow. Aktuálny plný
    verifier prešiel `226/226` v Chromium, Firefox aj Edge, vrátane factory
    preset auditu `199/199`; staré `224/224` a contention `213/225` výsledky
    ostávajú iba historickou stopou. Zostávajú current-tree full Vitest,
    manuálny device a deploy gates.
...
`dfaf230` synchronizuje jeho generated PRISM worklet. Release candidate HEAD is
`b8c7a00` (current implementation); the worktree is clean after reconciling
`WIP-01`. Je to release bookkeeping, nie dôkaz, že verejný deploy alebo celý KYX
release je hotový. Každý ďalší agent
...
| Browser timing measurement + release evidence       | `src/browser-checks.ts`, `scripts/verify-browser.mjs`, `AGENT_WORK_LOG.md`, `RELEASE_READINESS_REPORT.md`, tento roadmap                                                                                                                                                                                                                      | current Chromium, Firefox and Edge full verifier `226/226` per engine; factory preset audio `199/199`; zero-amount held-voice modulation guard passed `diff=0.1507`, `early=0.0000`, `late=0.1507`; latency, PRISM determinism, multi-instance CPU, template performance and production browser smoke pass; build/preflight/server smoke pass; current candidate full Vitest `239/2351/103` PASS; `WIP-01` is committed in `b8c7a00` | **AUTOMATED BROWSER + TEST GATES GREEN; OWNER GATES REMAIN** manual device/deploy/formatting decisions                                                             |
...
- PRISM crossover/render-quality v `a768595` pridáva kanonické LR2/LR4/LR8
  ...
  generated `public/fxeq-worklet.js` review, typecheck, build a current
  Chromium/Firefox/Edge `226/226` acceptance už prešli; current full Vitest
  `239/2351/103` is green and owner gates zostávajú otvorené;
- PRISM UI v `2242541` dokončuje Q2 tempo-sync enum rendering ako hudobné
  ...
  `18/18`; current browser verifier is `226/226` in Chromium/Firefox/Edge and
  factory preset audio is `199/199`. The current full Vitest run passed all
  soak tests and isolated performance is `3/3`. Zostávajú owner/manual/deploy/
  formatting rozhodnutia;
...
| full Vitest (current candidate)                             | **PASS — 239 files, 2351 passed, 103 skipped, 2454 total** in 424.94s; includes browser-compat, FXEQ performance, all 300 s FXEQ/VLYX/VØID soaks and no threshold changes |
| `npm run build` + worklet buildy + bundle budget           | PASS — candidate build after `b8c7a00`: entry 946 KB / 995 KB, total JS chunks 1863 KB / 2400 KB, core worklets 98 KB / 120 KB, 359 modules; PWA precache 57 entries / 4052.78 KiB; lazy ranker WASM is 13.6 MB and excluded from the app-shell precache |
...
| `npm run test:browser`                                     | **PASS — 226/226 Chromium; 226/226 Firefox; 226/226 Edge**; same full verifier passed per engine, including factory audio, real worklet DSP, PDC, collaboration, embed/share, touch and plugin workflow; no threshold changes                                                                                                                                                                          |
| current lifecycle/async regression batch                  | **PASS — 35/35**; closed-context recreation/worklet refresh lock, recorder partial-start/stop/cancel cleanup, onset Worker fallback/copy/filter/abort, factory preset audio quality and preset routing                                                                                                                                                                          |
...
| Post-`cb1bde7` hardening regression batch                  | PASS — 48/48 targeted tests + 300 s PRISM soak (6.0 MB heap growth, −0.003 dB drift, zero non-finite samples, zero tail peak); current full attempt also passed the 300 s FXEQ/VLYX/VØID soaks                                                                                                                                                                      |
...
`2320` testov prešlo, `103` bolo zámerne skipped (`2423` total) v single-worker
  behu za `2304.38s`. Staršie červené behy nižšie zostávajú historickým evidence
...
- [x] deterministický PRISM/FXEQ export (core, host seed aj browser
      `serialize → reload → offline bounce`; 224/224 Chromium gate),
...
- [x] PRISM morph/undo/sidechain host workflow — implementované a browser-
      accepted v aktuálnom candidate; `224/224` quiet Chromium a full Vitest
      `239/2351/103` prešli, manuálny/device/deploy release kroky zostávajú,
...
3. browser test missing-model/hash-mismatch/timeout fallbacku je hotový
   (`224/224`); reload/offline-cache a device cold-start evidence ostáva pre
   P2.5 device gate;
...
- [x] browser-level test `serialize → reload → offline bounce` beží v
      ...
      round-tripe ostali pod max-sample toleranciou `1e-5` (Chromium gate 224/224).
...
uprostred `OfflineAudioContext`; scorepack však v `dad84f3`/`a9dd506` zachováva
cue duration po resamplingu, rešpektuje CANCEL medzi stages a počas manifest/ZIP
fázy a neprehĺta už skutočné render/decode/OOM chyby ako „missing asset“. Panel
vysvetľuje túto hranicu aj frame-granularitu video kontajnera.
