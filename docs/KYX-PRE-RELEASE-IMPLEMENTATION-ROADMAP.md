...
cancel-safe Worker fallback for transient navigation and slicing. The candidate
commits are clean and the current worktree is clean. The current full browser
verifier is green in Chromium, Firefox and Edge at `226/226` per engine; the
factory-only verifier is `199/199`. The authoritative full Vitest baseline
before `5b2bf5b` remains `236/2320/103`; the current-tree full run is tracked
below. The post-candidate scorepack regression is `4/4`.
Release approval still waits for a clean full-suite performance capture, the
manual device matrix, production deploy smoke and the formatting decision.
...
`dfaf230` synchronizuje jeho generated PRISM worklet. Current HEAD is `5b2bf5b`
and the worktree is clean after the lifecycle/async analysis hardening commit.
...
- PRISM crossover/render-quality v `a768595` pridáva kanonické LR2/LR4/LR8
  ...
  attempt has one remaining contention-sensitive performance outlier and
  owner gates zostávajú otvorené;
...
| full Vitest (current candidate attempt)                    | **2 failures — 236 files passed, 2334 tests passed, 103 skipped** in 730.58s; browser-compat assertion was fixed afterward and isolated FXEQ performance is 3/3, but the final full-suite performance capture remains open |
| `npm run build` + worklet buildy + bundle budget           | PASS — current candidate build after `5b2bf5b`: entry 943 KB / 995 KB, total JS chunks 1859 KB / 2400 KB, core worklets 98 KB / 120 KB, 359 modules; PWA precache 57 entries / 4048.41 KiB; lazy ranker WASM is 13.6 MB and excluded from the app-shell precache; worktree clean                                                                                                                  |
