...
- 2026-09-20 — **FM band-limiting (BLEP modulátor)**: FM square/triangle modulátor ide cez `shapeOscillator` tabuľku na schedule ratiu (sine passthrough; RATIO automation prelaďuje frekvenciu naživo so zamrznutým spektrom ako glide); odstráni vlastný aliasing modulátora AJ jeho seedovanie skladaných FM sidebandov. Keys netreba (čisto sínusové páry). Zvyšok: interakčné FM sidebandy sa nad Nyquistom sklápajú — DX7-fyzika zdieľaná s hardvérovými originálmi. Verifikácia: `tests/bandlimited.test.ts` 11/11 (+ FM wiring mock: square→1 tabuľka, sine→0; cielený count 6160 Hz → 3 harmonické).
- 2026-09-20 — **Preset loudness audit (RMS-konzistencia)**: `tests/preset-loudness-audit.test.ts` 8/8 — pokrytie (199 mapových záznamov kryje všetky inštrument/drumsynth presety, žiadne siroty; 6 drum pad presetov explicitne nemeraných — pady nenesú presetId, engine nemá kam gain aplikovať), pinnutý clamp-inventár (34), klasifikácia, finite/rounded sanity. Nálezy a opravy: `808.score.impact` bol vo fx rodine (token „impact" pred „808") → −18 dB na strope, po novom bass (+gain −18,0 → −9,0); `bass.house.pluck` bol v pluck rodine → −17,7, po novom bass (+gain → +1,8) — obe prepočítané exaktne z uložených meraní (gain = target − measured) + pravidlo v `catalog.ts` (bass-role nástroje vždy bass). Systematický nález: zvyšných 34 clamp-hitov (keys pady, granular cloudy, vocal swelly…) sú pomalé attacky — sonda meria integrované cez fixed window, teda attack rampu, nie hrateľný sustain (shortTermMax sa meria, ale nepoužíva); správna oprava je sustained-loudness sonda + re-measure v prehliadači (tu nie je k dispozícii), dovtedy pin. Pozorované bez akcie: Cinema Low (namerané +3,5 — dizajnovo horúce?), House Chords cluster, keys rodina systematicky ~13 dB nad targetom (na ľudské uši + re-measure). Verifikácia: preset sady 29/29.
- 2026-09-20 — **Hodnoty balík (taper + defaulty)**: `ParamDef.taper` + `taperToRatio`/`ratioToTaper` v `Slideri` — všetky Hz parametre s min > 0 (EQ, cutoffs, tones, xovery, dampy, LFO/woj rates) ťahajú logaritmicky (stred = geometrický priemer; klávesy krokuú multiplikatívne, type-in ostáva exaktný, fill kopíruje taper); bipolárne a OFF-at-0 (split, mono-bass, freq-shift, LFO rates s OFF) ostávajú lineárne konštrukciou helperov. VØID MIX default 100 → 25 (rack + node defaults; hlboký core default 100 ostáva ako legacy backfill; presety/goldeny používajú explicitné hodnoty). Audit ostatných MIX defaultov: saturation/character/buss = 1 správne (tón, nie blend), time-based ≤0,5, PRISM/VLYX 100 transparentné (moduly defaultne off). Verifikácia: `tests/slider-taper.test.ts` + `tests/ui/slider-taper.test.tsx` 14/14 (math, kontrakt, defaulty, drag/fill/klávesy); dotknuté sady 77 + 83 ✓; typecheck ✓ (registry/effects UI ostávajú v prettier inventári).
- 2026-09-20 — **AUTO-CHOP (record → auto-slice → pady)**: po resample take pristane tlačidlo `AUTO-CHOP "…" → PADS` — onsety z existujúceho worker-klienta (`detectTransientsAsync`), `slicesFromOnsets` v `transients.ts` (leading zero + dedupe + clamp + zero-cross snap; `zeroCrossSnap` presunutý zo SliceLabu do shared, bez duplicity) → `chopSampleToPads` s diagonálnym patternom v jednom undo kroku (rovnaká cesta ako manuálny SliceLab flow). Bez drum stopy poctivá hláška, tichý take hlási „no transients". Verifikácia: `tests/transients.test.ts` +3, `tests/ui/ExportPanel.test.tsx` end-to-end s mock recorderom (REC → STOP → chop command + note); SliceLab/commands regresia ✓.
