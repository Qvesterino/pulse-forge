...
const tracks = doc.tracks.map((track) => {
  ...
  let next = track;
  if (next.effects === undefined) { next = { ...next, effects: [] }; changed = true; }
  if (next.params === undefined) { next = { ...next, params: defaultInstrumentParams(next.instrument) }; changed = true; }
  else {
...
if (scenes === undefined) { scenes = [{ id: "_scene", name: "Scene A", patternId: withTracks.activePatternId }]; changed = true; }
...
if (filteredScenes.length !== scenes.length) { validScenes = filteredScenes; changed = true; }
const sceneIds = new Set(validScenes.map((s) => s.id));

let arrangement = withTracks.arrangement;
if (arrangement === undefined) { arrangement = { clips: [] }; changed = true; }
const sortedClips = [...arrangement.clips].filter((c) => sceneIds.has(c.sceneId) && c.startBar >= 0 && c.lengthBars >= 1).sort((a, b) => a.startBar - b.startBar);
...
if (filteredAutomation.length !== automation.length) { automation = filteredAutomation; changed = true; }
...
if (filteredLfos.length !== lfos.length) { lfos = filteredLfos; changed = true; }

let macros = withTracks.macros;
if (!Array.isArray(macros) || macros.length === 0) { macros = []; changed = true; }
let returns = withTracks.returns;
if (!Array.isArray(returns)) { returns = []; changed = true; }
let master = withTracks.master;
if (master === undefined || typeof master !== "object") { master = {}; changed = true; }
...
const withCompositionBase = (scenes !== withTracks.scenes ||
...
const patterns = withCompositionBase.patterns.map((pattern) => {
  let next = pattern;
  if (next.notes === undefined) { next = { ...next, notes: {} }; changed = true; }
  let rows = next.rows;
