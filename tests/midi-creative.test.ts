...
describe("MIDI creativity", () => {
  ...
  it("reverses, inverts, halves and doubles selected timing", () => {
    ...
    expect(halveNotes(source, 1920).map((value) => [value.start, value.duration])).toEqual([
      [0, 60],
      [120, 60],
    ]);
    expect(doubleNotes(source, 480).map((value) => [value.start, value.duration])).toEqual([
      [0, 240],
      [479, 1],
    ]);
  });
  ...
  it("applies scale lock, gate and strum deterministically", () => {
    ...
    expect(strumNotes(source, { spreadTicks: 120, direction: "up" }, 1920).map((value) => value.start)).toEqual([
      120, 0, 60,
    ]);
  });
  ...
  it("uses a stable seed for humanize and velocity randomization", () => {
    ...
    expect(
      randomizeVelocity(source, { amount: 2, seed: "same" }, 480).every(
        (value) => value.velocity >= 0.05 && value.velocity <= 1,
      ),
    ).toBe(true);
  });
  ...
  it("materializes seeded arpeggios across chord groups", () => {
    ...
    const first = arpeggiateNotes(
      source,
      { mode: "random", rateTicks: 120, octaveRange: 1, gate: 0.5, seed: "arp" },
      1920,
      (index) => `arp-${index}`,
    );
    const second = arpeggiateNotes(
      source,
      { mode: "random", rateTicks: 120, octaveRange: 1, gate: 0.5, seed: "arp" },
      1920,
      (index) => `arp-${index}`,
    ...
    expect(
      arpeggiateNotes(
        source,
        { mode: "down", rateTicks: 120, octaveRange: 0, gate: 1, seed: "x" },
        1920,
        (index) => `arp-${index}`,
      ).map((value) => value.pitch),
    ).toEqual([67, 64, 60, 67, 64, 60, 67, 64]);
  });

  it("creates note repeats with velocity falloff and clamps overflow", () => {
    const result = repeatNotes(
      [note("a", 60, 420, 240, 0.8)],
      { rateTicks: 60, count: 8, velocityFalloff: 0.2 },
      480,
      (index) => `repeat-${index}`,
    );
  ...
  it("generates rotated Euclidean rhythms", () => {
    ...
    const result = euclideanNotes(
      [note("root", 48, 0, 1920)],
      { pulses: 3, steps: 8, rotation: 1, pitch: 36, velocity: 0.75, gate: 0.5 },
      1920,
      (index) => `euclid-${index}`,
    );
  ...
  it("infers chord roots for a clamped bassline", () => {
    ...
    expect(result.map((value) => [value.pitch, value.start, value.duration])).toEqual([
      [24, 0, 240],
      [29, 960, 240],
    ]);
