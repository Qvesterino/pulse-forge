...
1. **Model nikdy negeneruje priamo do projektu.** Vždy vzniká _proposal_
...
sa pripojať ako _proposal source_ — žiadne priame mutácie projektu.
...
- Testy: `tests/favorites-ledger.test.ts` (11 — ledger v2, drum konverzia,
  ...
  `tests/ranker-favorites.test.ts` (3 — C2 preference groups: winner + 3
  siblings, features.v1 ×54, determinizmus; QA-1 presunul builder do
  `src/intent/ranker-favorites.ts` aby ho kryli vitest aj tsc).
...
- **P3 HARMÓNIA 2.0 (2026-09-23)**: knižnica 12 → **36 progresií** (core žánre
  ...
  vlastný akord). Pri 64 krokoch + default density sa výstup nemení (žiadny 4. bar, žiadne passing) — golden baselines zelené. Testy:
...
**Naučené váhy reranku (GOAL 35, `src/intent/rerank-weights.ts` + `npm run rerank:fit`)**: learning slučka uzavretá aj na VÝBERE — fit harness regeneruje bank každej ★ rolky (deterministický seed), renderuje finalistov, skóruje audio-fit a čistý grid search (`fitRerankWeight`) nájde audio váhu, ktorá by VYBRALA kept kandidáta najčastejšie. Inštalácia = `localStorage.setItem("pf:rerank-weights", ...)` (report `scripts/data/rerank-weights.json`); ranking-v3 váhový reťazec: explicit option → learned → default 0.3.
...
**PRODUCENTSKÝ DIALÓG (GOAL 40b, `src/intent/producer-session.ts`)** — producent si pamätá session: **decisions** (genre/bpm/key/mood/style z každej generácie), **follow-upy** („ten istý, len pomalšie" → bpmRange −6; „ale tvrdší" → energy up + aggressive; „ešte raz" → reroll) resolved PRVÉ v ⚡ DO IT chain, **varianty B/C** (tmavšie / energetické chips po generácii, one-shot patch). HUD chip `🎛 techno · 132 BPM · E Minor` + ✕ reset.
...
- **Vibe**: „popovejšie", „more pop" → kompozitný production intent (brighter 0.7 + punchier 0.6 + wider 0.5 na všetky tracky) — beží cez EXISTUJÚCI production planner.
  Router priority: loudness → **fader → tempo → vibe** → effectIntent → production → mix → revise → pattern. Všetko one-undo-per-command, panel status report.

**VOICE IDEA — producent počúva teba (GOAL 36, `src/intent/voice-idea.ts`)**: 🎤 IDEA — nahraj hlasom nápad (MediaRecorder → decode → 16 kHz mono) → **tvoje tempo** (flux autokorelácia funguje na slabičné pulzy) + **tvoj tón** (Goertzel chroma na hlase) → patch (bpmRange/key), a **tvoja melódia**: YIN pitch tracker + `framesToNotes` → kvantizované noty v tvojom tóne. ♪ SONG ich nalepí ako LEAD každej lead sekcie (`ComposeHum` → transpozícia do tónu skladby, `tileNotesAcrossPattern` tiling, winner = `resolveLeadTrackId`). Solo umelec: zahučá hook → dostane celý beat pod svoj hlas.

**Groove extraction (GOAL 34, `src/intent/groove-extraction.ts`)**: 🎧 REF → 🥁→DRUMS — **transkripcia groove do reálnych rows**: vlastný onset detektor (5 ms RMS flux, 3× medián threshold) → fázovo ladená kvantizácia na 16-step grid pri detekovanom BPM → klasifikácia každej rany do pásma z okna okolo onsetu (low = kick, mid = snare/clap, high = haty; low-pass ratio + ZCR) → `grooveRowsForPads` mapuje bandy na SKUTOČNÉ pady cez ich role. Inštalácia = `replacePatternInPlaceCommand` (one undo). Velocity = lokálny peak normalizovaný 0.35–1.

**Ranking v3 + referenčné tempo/tón (GOAL 33)**: 🎧 referenčný WAV teraz dáva aj **tempo** (onset autokorelácia, `src/ai/audio-tempo-key.ts` → bpmRange ±2) a **tón** (Goertzel chroma × Krumhansl profily → key). **Ranking v3** (`src/intent/ranking-v3.ts`, `rerankTopBySound`): dvojštupňový výber — ranker poradie → top-3 finalistov RENDERUJÚ sa → `scoreCandidatesBySound` vs žánrové targety → vážený mix (0.7 ranker / 0.3 audio) preusporiada hlavu banku; winner = bank[0] (proposal poukazuje naň). Degráda: render fail = pôvodné poradie. Pipeline `GenerateAsyncOptions.sound`; panel GENERATE ho posiela, winner marker podľa bank[0].
...
**HYBRID V3 + AKTIVÁCIA (GOAL 32, `src/ai/symbolic/prior-features-v3.ts`)**: v3 = semantic(16) ++ celý v1 riadok(44) = **60-dim** — OBA kanály. Model `symbolic-prior-v3.onnx` (valAUC **0.920** v manifeste — 0.909 pri GOAL 32 tréningu, odvtedy retrénovaný; 23.9 kB, kind `drums-v3`), chain `npm run prior:v3`. A/B verdikt **RECOMMEND-ON**: mood-only páry v3 0.53/0.46 (v1 slepý — 0), style-varying páry v3 **2.08/3.07 — ostrejšie než v1** (kanály sa sčítavajú). **`pf:embedding-conditioned` je od GOAL 32 default ON** — runtime degraduje v3 → v2 → v1 per candidate pri každej nedostupnosti. Provider chain: `runPriorGridV3` → `runPriorGridV2` → `runPriorGrid`.

**Embedding shadow A/B (GOAL 30, `npm run embedding:ab`)** — HISTÓRIA, prekonané GOAL 32 (riadok vyššie): model-level gate (ort-web vo vite serveri) — **v2 je komplementárny, nie dominantný**: mood-only páry v1 SLEPÝ (dist 0) / v2 vidí (0.54); style-explicit páry v1 ostrejší (1.44 vs 0.54). Verdikt KEEP-OFF platil pre čisté v2; predpovedaná cesta k ON (hybrid v3 conditioning) sa realizovala a flag `pf:embedding-conditioned` je od GOAL 32 default ON. Ponechané ako metodika A/B brány, nie ako aktuálny stav.
...
_Posledná úplná revízia mapy: 2026-09-19; P1 docs-sync audit 2026-09-23
