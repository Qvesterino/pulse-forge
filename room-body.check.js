  ...
  const renderDoc = async (doc) => {
    try {
      ...
      try {
        const served = await fetch("/src/rendering/renderer.ts").then((r) => r.text());
        const lines = served.split("\n");
        console.log("[room] SRC158:", (lines[157] || "").trim().slice(0, 180));
        console.log("[room] SRC159:", (lines[158] || "").trim().slice(0, 180));
        console.log("[room] SRC160:", (lines[159] || "").trim().slice(0, 180));
      } catch {}
      console.log("[room] FAIL", err.message);
      throw err;
    }
  };
