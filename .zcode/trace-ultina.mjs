...
const ROOT = "D:/VocalForge_DAW/plugins/ultina/src";
const starts = ["dsp/ultinaProcessor.ts"];
const seen = new Set();
const queue = [...starts];
...
while (queue.length) {
  ...
  while ((m = re.exec(src))) {
    ...
    if (relTarget.startsWith("analysis/") || relTarget.startsWith("editor/")) {
      external.add("DEFERRED(" + relTarget.split("/")[0] + "): " + relTarget);
      continue;
    }
...
const dsp = [...seen].filter((f) => !f.includes("/")).length;
